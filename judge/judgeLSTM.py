...
class JudgeLSTM(nn.Module):
    def __init__(self, num_classes=2, hidden_dim=64, window_size=3, lstm_layers = 1, num_heads=1, dropout=0.5):
        super(JudgeLSTM, self).__init__()
        self.window_size = window_size
        self.hidden_dim = hidden_dim

        # LSTM Layer
        self.lstm = nn.LSTM(
            input_size=3,  # Input dimension (e.g., prev, curr, duration)
    ...
    def forward(self, x):
        ...
        # Pass through LSTM
        lstm_output, (hidden, cell) = self.lstm(x)  # lstm_output: (batch_size, sequence_length, hidden_dim)

        # Apply attention
        attn_output, _ = self.attention(lstm_output, lstm_output, lstm_output)  # Self-attention

        # Aggregate attention output (e.g., take the mean over the sequence dimension)
        attn_output = attn_output.mean(dim=1)  # Shape: (batch_size, hidden_dim)

        # Pass through the classifier
        return self.classifier(attn_output)
        
    def train_model(self, data_file, criterion, optimizer, scheduler=None, num_epochs=10, batch_size=2, test_data=None, wandb_plot=True, random=True):  
